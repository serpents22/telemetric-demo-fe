<template>
  <alert :message="createDeviceStatus.message" :modalActive="modalActive" :isError="createDeviceStatus.isError" @close="closeNotification" />
          ...
          <div class="modal-content">
            ...
            <VeeForm :validation-schema="schema" v-slot="{ handleSubmit }" as="div" ref="form">
              <form @submit="handleSubmit($event, onSubmit)" class="form-wrapper">
                <BasicInput name="name" type="text" placeholder="Device name" :label="t('deviceName')" />
                <BasicInput name="code" type="text" placeholder="Serial number" :label="t('serialNumber')" />
                <BasicInput name="password" type="text" placeholder="Password" label="Password" />
                <BasicInput v-model="latitude" name="latitude" type="text" placeholder="Latitude" :label="t('latitude')" />
                <BasicInput v-model="longitude" name="longitude" type="text" placeholder="Longitude"
                  :label="t('longitude')" />
                <div class="w-fit">
                  <BasicButton type="button" class="outlined" :label="t('getLocation')" @click="getLocation()" :loading="getLocationLoading" />
                </div>
...
const schema = yup.object({
  ...
  code: yup.string().required().label(' '),
...
const { createDeviceStatus } = storeToRefs(useDevicesStore())
...
const onSubmit = async (values, { resetForm }) => {
  ...
  if (regButtonClick.value == 2) {
    await devicesStore.createDevice(values)
    modalActive.value = true
    if (createDeviceStatus.value.isError) {
